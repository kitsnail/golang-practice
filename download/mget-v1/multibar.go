...
func MultiBarNew() (*BarContainer, error) {
...
func (b *BarContainer) MakeBar(total int64, prepend string) ProgressFunc {
	ch := make(chan int64)
	bar := &ProgressBar{
		Width:           int64(b.screenWidth - len(prepend) - 20),
		Total:           total,
		Prepend:         prepend,
		LeftEnd:         '[',
		RightEnd:        ']',
	...
	return func(progress int64) { bar.progressChan <- progress }
}
